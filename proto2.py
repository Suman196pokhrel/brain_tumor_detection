...
class Ui_MainWindow(object):
    ...
    def load_table_data(self):
        ...
        for items in data:
            ...
            self.tableWidget.setItem(
                rowPosition, 0, QtWidgets.QTableWidgetItem(f"{items[0]}"))
            self.tableWidget.setItem(
                rowPosition, 1, QtWidgets.QTableWidgetItem(f"{items[1]}"))
            self.tableWidget.setItem(
                rowPosition, 2, QtWidgets.QTableWidgetItem(f"{items[2]}"))
            self.tableWidget.setItem(
                rowPosition, 3, QtWidgets.QTableWidgetItem(f"{items[3]}"))
            self.tableWidget.setItem(
                rowPosition, 4, QtWidgets.QTableWidgetItem(f"{items[4]}"))
            self.tableWidget.setItem(
            ...
            self.tableWidget.item(rowPosition, 1).setBackground(
                QtGui.QColor(185, 255, 187))
            self.tableWidget.item(rowPosition, 2).setBackground(
                QtGui.QColor(185, 255, 187))
            self.tableWidget.item(rowPosition, 3).setBackground(
                QtGui.QColor(185, 255, 187))
            self.tableWidget.item(rowPosition, 4).setBackground(
                QtGui.QColor(185, 255, 187))
            self.tableWidget.item(rowPosition, 5).setBackground(
                QtGui.QColor(185, 255, 187))
    ...
    def add_data(self):
        ...
        image1 = self.lineEdit.text()
        # print("Image Path desc => ", image1 , type(image1))
        ...
        try:
            connection1.execute(
                f"INSERT INTO btd.patient (first_name,last_name,age,gender,contact_number,address,image) VALUES ('{fname}','{lname}',{age},'{gender}','{contact_num}','{address}','{image1}')")
    ...
    def update_data(self):
        ...
        if(id != '' and id != " "):
            connection1.execute(
                f"UPDATE btd.patient SET first_name = '{fname}',last_name = '{lname}',age = {age},gender = '{gender}',contact_number = '{contact_num}',address = '{address}', image= '{image}' WHERE id={id}")
            # data = connection1.myCursor.fetchall()
    ...
    def open_file_selector(self):
        ...
        fname = QFileDialog.getOpenFileName(
            self.centralwidget, 'Open File', 'c\\', 'Image files (*.png *.jpg *.jpeg)')
        imgPath = fname[0]
        print(imgPath)
    ...
    def messageBox(self, text):
        ...
        if(text == 'delete'):
            ...
            msg.exec_()
        elif(text == 'missing Image'):
            msg.setText(f"Please provide input image")
            msg.setIcon(QMessageBox.Critical)
            msg.exec_()
        elif(text == 'error'):
            msg.setText(f"An Error Occoured")
            msg.setIcon(QMessageBox.Critical)
            msg.exec_()

    # prediciton function

    def areFieldsEmpty(self):
        return not(self.lineEdit_2.text!="" and self.lineEdit_4.text!="" and self.lineEdit_6.text!="" and self.lineEdit_5.text!="" and self.lineEdit_7.text!="")
